...
from utils import *
from loss import OriTripletLoss, TripletLoss_WRT, FocalLoss, modality_center_loss
...
if args.noise_kernel > 0:
    suffix = suffix + '_noise_k{}'.format(args.noise_kernel)
else:
...
def train(epoch):
    ...
    for batch_idx, (
        ...
        pedestrian_mask = torch.cat((vis_pedestrian_mask, ir_pedestrian_mask), 0).cuda()
        data_time.update(time.time() - end)

        # 前向传播 - Pure数据
        feat, out, heatmaps = net(vis_pure_aug, ir_pure_aug)
        ...
        if epoch >= 65:
            ...
            loss = loss_id + loss_tri + 0.4 * loss_F + 1.7 * loss_HC + 0.24 * loss_twCompact
        else:
            loss = loss_id + loss_tri + 0.4 * loss_F
        # 反向传播
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        F_loss.update(loss_F.item(), batch_size)
        if epoch >= 65:
        ...
        if batch_idx % 50 == 0:
            ...
            if epoch >= 65:
                print('HC: {HC_loss.val:.4f} ({HC_loss.avg:.4f}) '
                      'twC: {twCompact_loss.val:.4f} ({twCompact_loss.avg:.4f}) '.format(HC_loss=HC_loss,
                                                                                         twCompact_loss=twCompact_loss))
