...
class embed_net(nn.Module):
    def __init__(self, class_num, gm_pool='on', arch='resnet50', dataset='sysu'):
        ...
        if self.dataset == 'regdb':
            # For RegDB, remove layer4, pool_dim becomes 1024 (layer3 output)
            # MultiScaleModule takes 512 channels (layer2 output), placed BEFORE layer3
            pool_dim = 1024
            self.multiscale = MultiScaleModule(512)
        else:
            # For SYSU/LLCM, keep layer4, pool_dim is 2048
            # MultiScaleModule takes 1024 channels (layer3 output), placed AFTER layer3
            pool_dim = 2048
    ...
    def forward(self, vis, ir, modal=0):
        ...
        if self.dataset == 'regdb':
            # RegDB: MultiScaleModule (CSFP) before layer3, NO layer4
            x = self.multiscale(x)
            x = self.base_resnet.base.layer3(x)
        else:
            # SYSU/LLCM: layer3 → MultiScaleModule (CSFP) → layer4
            x = self.base_resnet.base.layer3(x)
            x = self.multiscale(x)
