...
# ========== SCG Loss: Soft Consistency Guidance (BCE + Dice) ==========

class FocalLoss(nn.Module):
    """SCG (Soft Consistency Guidance) 损失
    通过 BCE + Dice 损失对 FRL 输出的前景热力图与掩码进行软一致性监督。
    """
    ...
    def forward(self, mask, heatmaps):
        """
        Args:
            mask:     [B, 1, H, W] 二值行人掩码 (ground truth)
            heatmaps: [B, 1, H, W] FRL 输出的前景热力图 (Sigmoid 后)
        Returns:
            loss: BCE + Dice
        """
        # BCE Loss (像素级监督)
