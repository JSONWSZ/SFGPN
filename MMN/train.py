...
from data_loader import SYSUData_mask, SYSUData, RegDBData, TestData
from data_manager import *
from eval_metrics import eval_sysu, eval_regdb
from model import embed_net
from utils import *
from loss import OriTripletLoss, TriLoss, DCLoss, FocalLoss
...
parser.add_argument('--scg_weight', default=0.008, type=float, help='SCG loss weight (default: 0.008)')
...
suffix = suffix + '_scg{}'.format(args.scg_weight)
...
# 训练集 transform: 仅包含 ToPILImage + ToTensor + Normalize
# Pad/Crop/Flip/Erasing 在 data_loader 的 numpy 层面处理（同步图像+掩码）
...
if dataset == 'sysu':
    # 使用掩码感知的数据集，加载 SFGPN 数据路径下的 pure 图像和掩码
    trainset = SYSUData_mask(data_path, transform_train=transform_train)
    # generate the idx of each person identity
    color_pos, thermal_pos = GenIdx(trainset.train_color_label, trainset.train_thermal_label)

    # testing set (使用原始测试图像路径)
...
def train(epoch):
    ...
    for batch_idx, (input1, vis_mask, label1, input2, ir_mask, label2) in enumerate(trainloader):

        labels = torch.cat((label1, label1, label2, label2), 0)

        input1 = Variable(input1.cuda())
        input2 = Variable(input2.cuda())
        vis_mask = Variable(vis_mask.cuda())
        ir_mask = Variable(ir_mask.cuda())

        labels = Variable(labels.cuda())
        data_time.update(time.time() - end)

        # 前向传播: 现在返回 11 个值 (原9个 + [xo, gray] + heatmaps)
        feat1, feat2, feat3, feat4, out1, out2, out3, out4, g1, heatmaps = net(input1, input2)
        ...
        # ===== SCG 损失: 热力图 vs 掩码 =====
        pedestrian_mask = torch.cat((vis_mask, ir_mask), 0)
        ...
        # 总损失 = 原MMN损失 + λ * SCG
        loss = loss_id + loss_tri + loss_dcl + args.scg_weight * loss_F
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        F_loss.update(loss_F.item(), 2 * input1.size(0))
        total += labels.size(0)
        ...
        if batch_idx % 50 == 0:
            print('Epoch: [{}][{}/{}] '
                  ...
                  'FLoss: {F_loss.val:.4f} ({F_loss.avg:.4f}) '
                  'Accu: {:.2f}'.format(
                epoch, batch_idx, len(trainloader), current_lr,
                100. * correct / total, batch_time=batch_time,
                train_loss=train_loss, id_loss=id_loss, tri_loss=tri_loss, F_loss=F_loss))

    writer.add_scalar('total_loss', train_loss.avg, epoch)
    writer.add_scalar('id_loss', id_loss.avg, epoch)
    writer.add_scalar('tri_loss', tri_loss.avg, epoch)
    writer.add_scalar('F_loss', F_loss.avg, epoch)
