...
class FocusModule(nn.Module):
    """
    FocusModule (FRL — Foreground Region Localization)
    通过上采样与双向条状空间注意力生成聚焦热力图，
    引导网络显式聚焦于行人身体区域。
    仅在训练时使用，推理时不增加计算开销。
    """
...
class embed_net(nn.Module):
    def __init__(self,  class_num, no_local= 'on', gm_pool = 'on', arch='resnet50', frl='on'):
        ...
        # FRL modules (one per modality, inserted after layer2)
        if self.frl == 'on':
    ...
    def forward(self, x1, x2, modal=0):
        ...
        if self.non_local == 'on':
            ...
            # ===== FRL INSERTION (after layer2 + non-local) =====
            if self.frl == 'on' and modal == 0:
                B = x.shape[0] // 2
                feat_vis = x[:B]
                feat_ir = x[B:]
            ...
            # no non-local: simpler forward through base_resnet layers
            x = self.base_resnet.base.layer1(x)
            x = self.base_resnet.base.layer2(x)

            # ===== FRL INSERTION (after layer2, no non-local) =====
            if self.frl == 'on' and modal == 0:
                B = x.shape[0] // 2
                feat_vis = x[:B]
                feat_ir = x[B:]
