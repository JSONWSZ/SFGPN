...
def pick_evenly_from_cams(img_lists, K):
    ...
    base = K // C         # 每个摄像头至少取 base 张
    remain = K - base * C # 剩余从序号靠前的摄像头继续取

    selected = []

    for idx, cam in enumerate(cams):
        take = base + (1 if idx < remain else 0)
        imgs = img_lists[cam][:take]  # 取前 take 张，不随机
...
def extract_embedding(img_path,modal=1):
    ...
    # 你的 a=0.5 → sqrt(0.5)=0.707 使用加权
    a = 0.5
    final_feat = a*f1 + (1-a)*f2

    # 模型已经 L2 norm，因此这里不需要再 norm
    return final_feat.cpu().numpy()
    # return feat_att[:1].cpu().numpy()
